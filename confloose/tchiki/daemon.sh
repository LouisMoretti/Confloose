#!/usr/bin/env bash
# tchiki daemon: loop tchiki.mp3 headless at >=75% volume; pop a recoloured face
# in a small floating window every few seconds. Needs bash ($RANDOM); on the
# fleet /bin/sh is bash, but env bash makes it work whatever /bin/sh points to.
...
FIRST_MIN=2;  FIRST_MAX=4      # seconds before the first face
WAIT_MIN=2;   WAIT_MAX=7       # seconds between faces
FLASH_MIN=2;  FLASH_MAX=4      # seconds a face stays
...
# Ignore the terminal's HUP; on stop, kill our children. INT/TERM must exit
# explicitly or the loop would resume after the handler.
trap '' HUP
cleanup() {
...
# Unmute and set the default sink to 75% (PipeWire: wpctl; fallbacks pactl, pamixer, amixer).
set_volume() {
    if command -v wpctl >/dev/null 2>&1; then
        ...
        pamixer --set-volume 75 2>/dev/null
    elif command -v amixer >/dev/null 2>&1; then
        amixer -q sset Master 75% unmute 2>/dev/null
    fi
}
...
# Recoloured, resized copy of $1 ($2x$3) via ImageMagick; the original if absent.
colorize() {
    im=$(command -v magick 2>/dev/null || command -v convert 2>/dev/null)
    if [ -z "$im" ]; then printf '%s' "$1"; return; fi
    out="$lockdir/flash.jpg"
    case $((RANDOM % 6)) in
...
# i3 tiles new windows: float and place ours (matched by title) once it is mapped.
place_window() {
...
flash() {
    ...
    w=$((RANDOM % 360 + 200)); h=$((w * 3 / 2))
    maxh=$((SCR_H - 30)); [ "$h" -gt "$maxh" ] && { h=$maxh; w=$((h * 2 / 3)); }
    ax=$((SCR_W - w)); [ "$ax" -lt 1 ] && ax=1
    ay=$((SCR_H - h)); [ "$ay" -lt 1 ] && ay=1
    x=$((RANDOM % ax)); y=$((RANDOM % ay))
    shown=$(colorize "$src" "$w" "$h")
    title="confloose-tchiki-$$-$RANDOM"
    timeout "${dur}s" feh --title "$title" --borderless --geometry "${w}x${h}" \
        --auto-zoom --hide-pointer --no-menus --image-bg black "$shown" >/dev/null 2>&1 &
    flash_pid=$!
    place_window "$title" "$w" "$h" "$x" "$y"
